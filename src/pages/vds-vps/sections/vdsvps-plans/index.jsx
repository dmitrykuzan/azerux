...
import {
  VDSVPS_ANNUALY,
  VDSVPS_ANNUALY_1GBPS,
  VDSVPS_PLANS,
  VDSVPS_PLANS_1GBPS,
  VDSVPS_QUARTERLY,
  VDSVPS_QUARTERLY_1GBPS,
  VDSVPS_SEMI,
  VDSVPS_SEMI_1GBPS
} from '@utils';
...
export const VdsVpsPlans = (props) => {
  ...
  //active dropdown
  const [toggleActive, setToggleActive] = useState('100mbps');
  //active toggle
  ...
  return (
    <section className="plans plans--vdsvps" id="plans">
      <Container>
        <div className="plans__wrapper">
          <div className="plans__top stack column">
            ...
            <div className="plans--host__navigation-wrapper stack align-center">
              <div className="plans__tabs-navigation stack align-center">
                ...
                >
                  {t.actions.annualy}
                  <span className="plans__tabs-button-sale stack center">-20%</span>
                </button>
              </div>
              ...
              <div ref={ref} className={`plans__tabs-dropdown ${active ? '' : 'active'}`}>
                ...
                <ul className="plans__tabs-dropdown-list stack column">
                  ...
                  <li className="plans__tabs-dropdown-item">
                    <button
                      className={`plans__tabs-button-mobile stack center ${
                        activeTab === 'semiannualy' ? 'active' : ''
                      }`}
              ...
              <div className="plans__toggle stack align-center">
                <Typography className="plans__toggle-label" tag="p" weight="300" upperCase>
                  {t.labels.bandwidth}
                </Typography>
                <div className="plans__toggle-body stack align-center">
                  <button
                    className={`plans__toggle-button ${toggleActive === '100mbps' ? 'active' : ''}`}
                    onClick={() => setToggleActive('100mbps')}
                  >
                    100 Mbps
                  </button>
                  <button
                    className={`plans__toggle-button ${toggleActive === '1gbps' ? 'active' : ''}`}
                    onClick={() => setToggleActive('1gbps')}
                  >
                    1 Gbps
                  </button>
                </div>
          ...
          {activeTab === 'monthly' && (
            <div className="plans__content stack">
              ...
              >
                {toggleActive === '1gbps'
                  ? VDSVPS_PLANS_1GBPS.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
                  ...
                  : VDSVPS_PLANS.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
          ...
          {activeTab === 'quarterly' && (
            <div className="plans__content stack">
              ...
              >
                {toggleActive === '1gbps'
                  ? VDSVPS_QUARTERLY_1GBPS.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
                  ...
                  : VDSVPS_QUARTERLY.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
          ...
          {activeTab === 'semiannualy' && (
            <div className="plans__content stack">
              ...
              >
                {toggleActive === '1gbps'
                  ? VDSVPS_SEMI_1GBPS.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
                  ...
                  : VDSVPS_SEMI.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
          ...
          {activeTab === 'annualy' && (
            <div className="plans__content stack">
              ...
              >
                {toggleActive === '1gbps'
                  ? VDSVPS_ANNUALY_1GBPS.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
                  ...
                  : VDSVPS_ANNUALY.map(({ id, tariff, price, list, featuresTitle, featuresList, label }) => (
                      <SwiperSlide key={id}>
                        <PlanCard
                          tariff={tariff}
                          price={price}
